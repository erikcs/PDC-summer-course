...
int main(int argc, char** argv)
{
	...
	int arrayLength = (M)*(M);
	int* iArray = calloc(arrayLength,sizeof(int));
        int* jArray = calloc(arrayLength,sizeof(int));
	if (iArray == NULL || jArray==NULL) {
...
//		#pragma omp parallel for
		for (int cnt = 0; cnt < arrayLength; cnt++)
		{
			...
			double J_new_local[2][threads][8];
                        for (int m=0; m<threads; m++)
			{
                                J_new_local[0][m][0] = J_new[0][iArray[cnt]];
				J_new_local[1][m][0] = J_new[1][iArray[cnt]];
                        }
			int mythread = omp_get_thread_num();
				
			double t0, t1;
			c0 = (1+r) * k[iArray[cnt]] + w*0 - k[jArray[cnt]];
			t0 = sqrt(c0) + B * ( p0 * J_old[0][jArray[cnt]] + p1 * J_old[1][jArray[cnt]] );
			c1 = (1+r) * k[iArray[cnt]] + w*1 - k[jArray[cnt]];
			t1 = sqrt(c1) + B * ( p0 * J_old[0][jArray[cnt]] + p1 * J_old[1][jArray[cnt]] );

			if ( c0 >= 0 && t0 >= J_new_local[0][mythread][0] )
				J_new_local[0][mythread][0] = t0;
			if ( c1 >= 0 && t1 >= J_new_local[1][mythread][0] )
				J_new_local[1][mythread][0] = t1;
			for (int m = 0; m < threads; m++)
			{
				if (J_new_local[0][m][0] > J_new[0][iArray[cnt]])
					J_new[0][iArray[cnt]] = J_new_local[0][m][0];
                                if (J_new_local[1][m][0] > J_new[1][iArray[cnt]])
                                        J_new[1][iArray[cnt]] = J_new_local[1][m][0];
			}
		}
	}
